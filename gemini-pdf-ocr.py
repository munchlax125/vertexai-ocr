...
EXTRACTION_FIELDS = [
    ...
    "퇴직연금세액공제", "연금계좌세액공제", "사업자 등록번호", "상호", "수입종류 구분코드",
...
### 데이터 추출 시 핵심 규칙
- **열 제목(Header) 기준 추출**: 데이터의 위치가 아니라, **반드시 표의 열 제목을 기준으로 값을 정확히 매핑**하세요. 예를 들어, '상호' 열 아래에 있는 값만 '상호' 필드에 넣어야 합니다.
- **"업종 코드"는 반드시 6자리 숫자입니다.** 다른 문자나 기호가 포함된 경우 숫자만 추출하고, 6자리가 아니면 "N/A"로 처리하세요.
- **데이터 누락 금지**: 모든 사업소득 행을 찾아 각각 별도의 JSON 객체로 만들어야 합니다.
...
def clean_business_code(value: str):
    """'업종 코드'를 정제하여 6자리 숫자 코드를 반환합니다. 유효하지 않으면 None을 반환합니다."""
    # 입력값이 문자열이 아니거나, 비어있거나, 공백만 있는 경우 즉시 None 반환
    ...
    # 6자리 숫자인지 확인하고, 맞으면 해당 값을, 아니면 None을 반환합니다.
